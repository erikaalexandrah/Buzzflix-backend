...
export class TmdbService {
  ...
  async discoverMoviesByReleaseDate(page: number = 1) {
    try {
      const url = `${this.tmdbBaseUrl}/discover/movie`;
      ...
      console.error(
        `Error discovering movies from page ${page}:`,
        error.response?.data || error.message,
      );
      throw new Error(`Failed to discover movies from page ${page}`);
    }
  }
  ...
  async getAllMovies() {
    ...
    while (page <= maxPages) {
      try {
        ...
        console.error(
          `Error fetching movies from page ${page}:`,
          error.response?.data || error.message,
        );
  ...
  async getLatestMovies() {
    try {
      ...
      if (!response.data || !response.data.results) {
        ...
        throw new Error(
          "Failed to fetch latest movies - invalid response format",
        );
      }
      ...
      const movies = await Promise.all(
        response.data.results.map(async (movie: any) => {
          try {
            ...
            const subtitles = movieDetails.spoken_languages
    ...
    while (page <= maxPages) {
      try {
        ...
        for (const movie of movies) {
          const movieDetails = await this.getMovieDetails(movie.id);
          movieDetails.actors.forEach((actorId: number) =>
